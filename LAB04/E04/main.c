...
void stampa_tratte(int, Tratta[MAX_SIZE]);
void stampa_tratta(Tratta);
void stampa_tratte_su_file(int, Tratta[MAX_SIZE], FILE*);
...
int main(int argc, const char * argv[]) {
	...
	int i = seleziona_dati(numero_tratte, tratte, comando, ordinato);
	...
	for	(;i == 1;){
		...
		i = seleziona_dati(numero_tratte, tratte, comando, ordinato);
	}
 ...
 */
void stampa_tratte(int numero_tratte, Tratta tratte[numero_tratte]){
 ...
 */
void stampa_tratte_su_file(int numero_tratte, Tratta tratte[MAX_SIZE], FILE* fp){
	for (int i = 0; i < numero_tratte; i ++) {
 ...
 */
int seleziona_dati(int numero_tratte, Tratta tratte[numero_tratte], comando_e comando, int* ordinato){
	
	
	
	switch (comando) {
		case stampa:
			stampa_log(numero_tratte, tratte);
			return 1;
		case data:
			ordina_data(numero_tratte, tratte);
			return 1;
		case codice:
			ordina_codice(numero_tratte, tratte);
			return 1;
		case partenza:
			ordina_partenza(numero_tratte, tratte);
			*ordinato = 1;
			return 1;
		case arrivo:
			ordina_arrivo(numero_tratte, tratte);
			return 1;
		case stazione:
			if (*ordinato)
...
void stampa_log(int numero_tratte, Tratta tratte[numero_tratte]){
	...
	if (c == 'c'){
		stampa_tratte(numero_tratte, tratte);
	} else if (c == 'f') {
		...
		if (fp != NULL)
			stampa_tratte_su_file(numero_tratte, tratte, fp);
		else
